const API_URL = "/api/v1/diagnosticos/";
...
async function cargarClientesDatalist() {
    ...
    const res = await fetch('/api/v1/clientes/?all=1', { headers: { "Authorization": "Bearer " + (window.token || "") } });
...
function asignarEventosModal() {
    ...
    document.getElementById("matriculaInputModal").addEventListener("input", async () => {
        ...
        const res = await fetch(`/api/v1/ordenes_trabajo/?matricula=${encodeURIComponent(matricula)}`, { headers: { "Authorization": "Bearer " + (window.token || "") } });
        const data = await res.json();
